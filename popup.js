function activeTab() {
  ...
  // Helper to validate and sanitize grade values
  function validateGrade(grade) {
    if (!grade || typeof grade !== 'string') return 'N/A';
    // Only allow valid grade letters: A+, A, B, C, D, F, N/A
    const validGrades = ['A+', 'A', 'B', 'C', 'D', 'F', 'N/A'];
    if (validGrades.includes(grade)) return grade;
    // If it starts with a valid letter, return just the letter
    const firstChar = grade.charAt(0);
    if (['A', 'B', 'C', 'D', 'F'].includes(firstChar)) {
      return firstChar === 'A' && grade.includes('+') ? 'A+' : firstChar;
    }
    return 'N/A';
  }
  ...
  function getGradeClass(grade) {
    const validated = validateGrade(grade);
    if (!validated || validated === "N/A") return "grade-na";
    const firstChar = validated.charAt(0);
  ...
  async function render(res) {
    ...
    if (report.grade) {
      ...
      const validatedGrade = validateGrade(grade.grade);
      const validatedScore = validateNumber(grade.score, 0, 100);
      const validatedPassCount = validateNumber(grade.passCount, 0, 999);
      const validatedWarnCount = validateNumber(grade.warnCount, 0, 999);
      const validatedFailCount = validateNumber(grade.failCount, 0, 999);
      const gradeClass = getGradeClass(validatedGrade);
      
      // Use safe DOM manipulation instead of innerHTML with template strings
      scoreDisplayEl.innerHTML = '';
      const scoreContainer = document.createElement('div');
      scoreContainer.className = 'score-with-grade';
      // gradeClass is validated and safe (contains only CSS class names like "grade-a")
      scoreContainer.innerHTML = `
        <div class="grade-letter-large ${gradeClass}">${escapeHtml(validatedGrade)}</div>
        <div class="score-content">
          <div class="score-label">SEO Score</div>
          <div class="score-value">${escapeHtml(validatedScore)} / 100</div>
          <div class="score-progress">
            <div class="score-progress-bar ${gradeClass}" style="width: ${escapeHtml(validatedScore)}%"></div>
          </div>
        </div>
      `;
      scoreDisplayEl.appendChild(scoreContainer);
      
      // Status badges - use safe DOM manipulation
      statusBadgesEl.innerHTML = '';
      const badges = [
        { count: validatedPassCount, label: 'Pass', class: validatedPassCount > 0 ? 'success' : 'neutral' },
        { count: validatedWarnCount, label: 'Warn', class: validatedWarnCount > 0 ? 'warning' : 'neutral' },
        { count: validatedFailCount, label: 'Fail', class: validatedFailCount > 0 ? 'fail' : 'neutral' }
      ];
      badges.forEach(badge => {
        const span = document.createElement('span');
        span.className = `badge ${badge.class}`;
        span.textContent = `${badge.count} ${badge.label}`;
        statusBadgesEl.appendChild(span);
      });
    } else {
      scoreDisplayEl.innerHTML = "";
      statusBadgesEl.innerHTML = "";
          ...
          try {
            ...
            const anchorData = problematicAnchors.map(a => {
              try {
                ...
                // Error extracting anchor data - return null to use fallback
                return null;
              }
            }).filter(Boolean);
...
Provide recommendations in JSON format:
{
  ...
  async function renderRecommendations(report, forceGenerate = false) {
      ...
      if (cached && !forceGenerate) {
        ...
        for (const rec of cached.recommendations || []) {
          ...
          if (issue && (issue.severity === 'fail' || issue.severity === 'warn')) {
            ...
            const escapedTitle = escapeHtml(rec.title || rec.issue || '');
            const escapedSeverity = escapeHtml(rec.severity || issue.severity || '');
            html += `
              <div class="recommendation-item ${escapedSeverity}">
                <div class="recommendation-header">${escapedTitle}</div>
    ...
    try {
      ...
      const recommendations = await getAIRecommendations(report, (attempt, maxRetries) => {
        if (attempt > 0 && apiKey) {
          recommendationsContent.innerHTML = `<div class="recommendation-loading">Rate limit hit. Retrying with AI... (Attempt ${escapeHtml(attempt + 1)}/${escapeHtml(maxRetries + 1)})</div>`;
        }
      });
      ...
      recommendationsContent.innerHTML = `<div class="recommendation-error">${escapeHtml(errorMessage)}</div>`;
      if (recommendationsPrompt) recommendationsPrompt.style.display = 'none';
    }
  }
  ...
  async function loadTheme() {
    try {
      ...
      // Error loading theme - default to light mode
      applyTheme('light');
    }
  }
  ...
  async function toggleTheme() {
    ...
    try {
      ...
      // Error saving theme - silently fail
    }
  }
  ...
  (async () => {
    ...
    try {
      ...
      // Error loading report - show empty state
      render(null); 
    }
  })();
